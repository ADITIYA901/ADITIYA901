...
import { Vote, Check, Eye, EyeOff, Mail, Lock, User, Fingerprint } from 'lucide-react';
...
export default function VoterSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signup(
        { email, password, name, aadhaar_id: aadhaarId.replace(/\s/g, '') },
        'voter'
      );
      toast.success('Voter registration submitted!');
      navigate('/voter/dashboard');
    } catch (err: any) {
      toast.error(err.message || 'Failed to register as voter');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="gradient-bg flex-1 flex items-center justify-center p-8 lg:p-16 relative overflow-hidden">
        ...
        >
          ...
          <h1 className="text-4xl lg:text-5xl font-bold text-white mb-4">
            BlockVote Voter Portal
          </h1>
          <p className="text-white/70 text-lg mb-10">
            Register to participate in secure, transparent elections. Your vote matters, and blockchain ensures it counts.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <Input
                ...
                value={aadhaarId}
                onChange={(e) => setAadhaarId(formatAadhaar(e.target.value))}
                icon={<Fingerprint className="w-5 h-5" />}
                error={
                  aadhaarId && !isValidAadhaar(aadhaarId)
                    ? 'Aadhaar ID must be 12 digits'
                    : undefined
                }
                required
              />
            ...
            <Button
              type="submit"
              isLoading={isLoading}
              disabled={!captchaVerified || password !== confirmPassword || !isValidAadhaar(aadhaarId)}
              className="w-full"
              size="lg"
            >
              Register as Voter
